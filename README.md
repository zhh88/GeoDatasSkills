...
- WKT 点、线、面
- GPX-like 轨迹
- Shapefile / GeoPackage / GeoTIFF / LAS-LAZ / KML / CityJSON / 3D Tiles / glTF 元数据识别
- 轨迹数据
- 传感器时序数据
- 图片、视频、文本、文档等多模态元数据
- API 返回的空间对象
...
geodataskills examples/sample_points.csv --html-report outputs/report.html
```

## 专业格式适配策略

专业 GIS / 三维格式通常需要较重的可选依赖。GeoDatasSkills 会先完成格式识别和 metadata-only 注册，并在报告中给出依赖建议。

| 格式 | 识别 | 深度转换建议 |
|---|---:|---|
| Shapefile | 支持 | `geopandas` / `pyshp` |
| GeoPackage | 支持 | `geopandas` |
| GeoTIFF | 支持 | `rasterio` |
| LAS/LAZ | 支持 | `laspy` |
| KML/KMZ | 支持 | `fastkml` |
| CityJSON | metadata-only | 后续可解析 CityObjects |
| 3D Tiles | metadata-only | 后续可解析 tileset 层级 |
| glTF/GLB | metadata-only | 后续可解析 mesh 与节点 |
...
```python
dataset = skill.ingest("examples/sample_points.csv")
skill.export_html_report(dataset, "outputs/report.html")
```
